...
- All the links in the robot must be rigid-bodies. Collision or weight settings is not mandatory, but is recommended for accurate physics simulation.
...
- Select `J1` and scroll down to `Revolute Joint` section. Click `Axis` and select Z. Repeat same process for `J4` and `J6`.
